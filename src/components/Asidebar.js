  ...
  const calendar = () => {
    ...
    if (currentRow.length > 0 && currentRow.length < 7) {
      while (currentRow.length < 7) {
        currentRow.push(<td className={selectedDate === dayCounter ? "orange" : ""}
          onClick={() => handleDateClick(dayCounter)} key={`empty-${currentRow.length}`} />);
      }
      rows.push(<tr key={`row-${rows.length}`}>{currentRow}</tr>);
    }
    ...
    return (
      <div onClick={toggleColor} className={changeColor[index] ? "storeMenu active" : "storeMenu"} key={index}>
        ...
        <div className="iconBox">
          <button onClick={() => reservation(mapData)} className="bottomBtn">예약하기</button>
